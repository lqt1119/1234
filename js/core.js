...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    // 礼物消息单独展示，text 只放进礼物卡片尾部
    const hasGift = msg.gift && msg.gift.name;
    let content = (!hasGift && msg.text) ? `<div>${msg.text.replace(/\n/g, '<br>')}</div>` : '';
    if (msg.image) {
    ...
    // 礼物消息：渲染专属卡片
    if (msg.gift && msg.gift.name) {
        const gTitle = (msg.sender === 'user' ? '我送出的礼物' : (settings.partnerName || '对方') + ' 送你的礼物');
        messageHTML += `<div class="gift-card ${msg.sender === 'user' ? 'gift--sent' : 'gift--received'}" title="礼物">
            <div class="gift-card-head"><i class="fas fa-gift"></i> ${gTitle}</div>
            <div class="gift-card-body">
                <div class="gift-card-icon">${msg.gift.icon || '🎁'}</div>
                <div class="gift-card-meta">
                    <div class="gift-card-name">${msg.gift.name}</div>
                    <div class="gift-card-price">¥${msg.gift.price || ''}</div>
                </div>
            </div>
            <div class="gift-card-foot">${msg.text || ''}</div>
        </div>`;
    }

    // 红包消息：渲染专属红包卡片（点击打开领取/退回）
    if (msg.redpacket && msg.redpacket.amount !== undefined) {
        const rp = msg.redpacket;
        const rpFromMe = msg.sender === 'user';
        const rpClass = rp.status || (rpFromMe ? 'sent' : 'pending');
        // 安全地把封面字符串传给 onclick（表情或 dataURL 都可能是长串，用 encodeURIComponent）
        const coverSafe = encodeURIComponent(rp.cover || '');
        const rpTitle = rpFromMe ? '我发的红包' : (settings.partnerName || '对方') + ' 发的红包';
        const rpStateTxt = rp.status === 'claimed' ? '已领取' : rp.status === 'returned' ? '已退回' : (rpFromMe ? '待领取' : '点击开启');
        messageHTML += `<div class="redpacket-card ${rpClass}" title="点击打开红包"
            onclick="window._openRedPacket && window._openRedPacket(${String(msg.id)}, ${rpFromMe}, '${coverSafe}')">
            <div class="rp-head"><i class="fas fa-envelope envelope"></i> ${rpTitle}</div>
            <div class="rp-body">
                <div class="rp-seal">${rp.cover || '🧧'}</div>
                <div class="rp-amt">¥ ${fmtMoney(rp.amount)}</div>
                <div class="rp-state">${rpStateTxt}</div>
            </div>
            ${rp.bless ? `<div class="rp-bless">${rp.bless}</div>` : ''}
        </div>`;
    }
...
const addMessage = (message) => {
    ...
    if (message.sender === 'user' && message.type === 'normal' && typeof window._onUserMessage === 'function') {
        ...
        window._pushGiftMessage = (opts) => {
            try {
                const o = opts || {};
                const isGive = !!o.give; // true=我送TA，false=TA送我的
                const msgId = Date.now() + Math.random();
                addMessage({
                    id: msgId,
                    sender: isGive ? 'user' : (settings.partnerName || '对方'),
                    text: o.text || '',
                    timestamp: new Date(),
                    status: isGive ? 'sent' : 'received',
                    type: 'normal',
                    gift: o.item ? { name: o.item.name, icon: o.item.icon, price: o.item.price } : null,
                });
                // 我送出的礼物/外卖：延迟模拟对方“查收”，随后标为已读
                if (isGive) {
                    const readDelay = 4000 + Math.random() * 5000; // 4~9 秒
                    setTimeout(() => {
                        try {
        ...
        // 推送一条对方发出的纯文本消息（用于 0~5min 内的延迟回复/留言回复）
        window._pushPartnerTextMessage = function(text) {
            try {
                if (!text) return true;
                addMessage({
                    id: Date.now() + Math.random(),
