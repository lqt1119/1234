...
const ICONS = {
    ...
    puzzle:   `<svg width="15" height="15" viewBox="0 0 15 15" fill="none"><path d="M3.5 2.5H9A1.5 1.5 0 0110.5 4v.8a1.1 1.1 0 101.5 1.5V6.5H10.5v5A1.5 1.5 0 019 13H3.5A1.5 1.5 0 012 11.5V4A1.5 1.5 0 013.5 2.5z" stroke="currentColor" stroke-width="1.2" fill="none"/><path d="M6 8.8L7.6 7.3l1.6 1.5M7.6 7.3l.2 2.6" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>`,
...
function _renderModernToolbar() {
    ...
    toolbar.innerHTML = `
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:10px 15px;border-bottom:1px solid var(--border-color);">
            ...
            <button class="toolbar-icon-btn" id="tb-puz-btn" title="拼字卡">
                ${ICONS.puzzle}
            </button>
        </div>
    ...
    toolbar.querySelector('#tb-puz-btn')?.addEventListener('click', _openPuzCardSettings);
 ...
 * 拼字卡（对方回复/留言自动组合）
 * ------------------------------------------------------------
 * 开关 + 滑块(最大条数 0~5)：开启后，全局所有“对方回复/留言”的地方
 * 会用你的字卡随机自由组合。每次拼出的条数在 0~最大条数 间随机，
 * 也可能一条都不拼（返回空 → 回落为默认文案）。
 * 全局暴露 composeGiftNote() 供 phone-home 等使用。
 * ============================================================ */
const PUZ_KEY = 'puzCardSettings';
function getPuzCardSettings() {
    try {
        const raw = localStorage.getItem(PUZ_KEY);
        if (raw) {
            const v = JSON.parse(raw);
            return { enabled: !!v.enabled, count: Math.max(0, Math.min(5, parseInt(v.count, 10) || 0)) };
        }
    } catch (e) {}
    return { enabled: false, count: 1 };
}
function savePuzCardSettings(s) {
    try { localStorage.setItem(PUZ_KEY, JSON.stringify(s)); } catch (e) {}
}
// 是否启用拼字卡（供外部判断是否需要回落默认文案）
window.puzzleCardEnabled = function() {
    try { return !!getPuzCardSettings().enabled; } catch (e) { return false; }
};

// 主字卡（启用且未被屏蔽）池：所有"对方回复/留言"都要从这里取内容
function buildReplyPool() {
    ...
    try { const raw = localStorage.getItem('disabledReplyItems'); if (raw) disabled = new Set(JSON.parse(raw)); } catch (e) {}
    const disabledGroupItems = new Set();
    (window.customReplyGroups || []).forEach(g => {
        if (g.disabled && Array.isArray(g.items)) g.items.forEach(it => disabledGroupItems.add(it));
    });
    return replies
        .filter(r => !disabled.has(r) && !disabledGroupItems.has(r))
        .map(r => String(r || '').trim())
        .filter(Boolean);
}
// 供外部获取主字卡启用池（对方消息来源），避免各模块重复实现过滤
window.getReplyCardPool = function() {
    try { return buildReplyPool(); } catch (e) { return []; }
};

// 从启用的主字卡池中随机抽 0~count 条并自由组合；返回字符串（未开启/0条 → ''）
window.composeGiftNote = function() {
    try {
        const st = getPuzCardSettings();
        if (!st.enabled || st.count < 1) return '';
        const pool = buildReplyPool();
        if (!pool.length) return '';
        // 条数在 0 ~ min(count,pool) 间随机，允许一条都不拼
        const max = Math.min(st.count, pool.length);
        ...
        for (let i = 0; i < n; i++) {
            let idx, tries = 0;
            do { idx = Math.floor(Math.random() * pool.length); tries++; } while (used.has(idx) && tries < 20);
            used.add(idx);
...
function _openPuzCardSettings() {
    const st = getPuzCardSettings();
    const overlay = document.createElement('div');
    overlay.id = 'puz-overlay';
    overlay.style.cssText = `
        position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;
        background:rgba(0,0,0,.45);padding:20px;animation:fadeIn .18s ease;
    `;
    overlay.innerHTML = `
        <div style="
            width:100%;max-width:360px;max-height:86vh;overflow:auto;box-sizing:border-box;
            background:var(--primary-bg,#fff);border:1px solid var(--border-color);
            border-radius:18px;padding:20px;box-shadow:0 16px 44px rgba(0,0,0,.25);
            font-family:var(--font-family);
        ">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                ${ICONS.puzzle}
                <span style="font-size:16px;font-weight:700;color:var(--text-primary);">拼字卡</span>
            </div>
            <div style="font-size:12px;color:var(--text-secondary);line-height:1.6;margin-bottom:16px;">
                开启后，所有对方回复/留言你的地方，都会用你的字卡随机组合。每次条数在 0~最大条数 间随机，也可能一条都不拼。
            </div>

            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                <span style="font-size:13px;color:var(--text-primary);font-weight:600;">启用拼字卡</span>
                <label style="position:relative;display:inline-block;width:48px;height:27px;flex-shrink:0;">
                    <input type="checkbox" id="puz-enabled" ${st.enabled ? 'checked' : ''} style="opacity:0;width:0;height:0;">
                    <span style="
                        position:absolute;inset:0;background:${st.enabled ? 'var(--accent-color,#c79a5a)' : '#cbd0da'};
                        border-radius:14px;transition:.2s;cursor:pointer;
                    "></span>
                    <span style="
                        position:absolute;top:3px;left:${st.enabled ? '24px' : '3px'};width:21px;height:21px;border-radius:50%;
                        background:#fff;transition:.2s;box-shadow:0 1px 3px rgba(0,0,0,.25);
                    "></span>
                </label>
            </div>

            <div id="puz-slider-wrap" style="opacity:${st.enabled ? 1 : .45};transition:.2s;margin-bottom:16px;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                    <span style="font-size:13px;color:var(--text-primary);font-weight:600;">最大拼字卡条数</span>
                    <span style="font-size:13px;font-weight:700;color:var(--accent-color,#c79a5a);">
                        <span id="puz-count-val">${st.count}</span> 条
                    </span>
                </div>
                <input type="range" id="puz-count" min="0" max="5" step="1" value="${st.count}"
                    style="width:100%;accent-color:var(--accent-color,#c79a5a);">
            ...
            <div id="puz-preview" style="
                background:var(--secondary-bg,#f5f5f7);border:1px dashed var(--border-color);
                border-radius:12px;padding:12px 14px;font-size:12px;line-height:1.7;
                color:var(--text-secondary);margin-bottom:16px;min-height:40px;
            "></div>

            <div style="display:flex;gap:10px;">
                <button id="puz-test" style="
                    flex:1;padding:11px;border:1.5px solid var(--border-color);border-radius:13px;
                    background:none;color:var(--text-secondary);font-size:13px;cursor:pointer;font-family:var(--font-family);
                ">随机预览</button>
                <button id="puz-save" style="
                    flex:2;padding:11px;border:none;border-radius:13px;
                    background:var(--accent-color,#c79a5a);color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:var(--font-family);
                ">保存</button>
            </div>
        </div>
    `;
    ...
    const enabledInput = overlay.querySelector('#puz-enabled');
    const countInput = overlay.querySelector('#puz-count');
    const countVal = overlay.querySelector('#puz-count-val');
    const sliderWrap = overlay.querySelector('#puz-slider-wrap');
    const preview = overlay.querySelector('#puz-preview');

    const refresh = () => {
        const on = enabledInput.checked;
        sliderWrap.style.opacity = on ? 1 : .45;
        countVal.textContent = countInput.value;
        preview.innerHTML = '';
    };
    const previewOnce = () => {
        const st2 = { enabled: enabledInput.checked, count: parseInt(countInput.value, 10) || 0 };
        savePuzCardSettings(st2);
        preview.innerHTML = window.composeGiftNote() || '<span style="opacity:.6">（未拼出内容，可能字卡库为空或已全部禁用）</span>';
    };
    enabledInput.addEventListener('change', refresh);
    countInput.addEventListener('input', refresh);
    previewOnce();

    overlay.querySelector('#puz-test').addEventListener('click', previewOnce);
    overlay.querySelector('#puz-save').addEventListener('click', () => {
        savePuzCardSettings({ enabled: enabledInput.checked, count: parseInt(countInput.value, 10) || 0 });
        overlay.remove();
        if (typeof showNotification === 'function') showNotification('拼字卡设置已保存', 'success');
    });
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
}
